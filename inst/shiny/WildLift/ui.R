      ...
      tabItem("multi1",
        fluidRow(
          column(width=12,
            ...
            HTML("<br/><p><strong>Limitations</strong> &mdash; Results using multiple levers are extrapolated based on knowledge from locations where single levers were studied. Combinations of these levers do not have documented examples and need to be treated with caution. Please see documentation on how combinations were parameterized.</p><p>MP = Maternity Penning; PE = Predator Exclosure; MR = Moose Reduction; WR = Wolf Reduction</p><p>Parameters for non-captive individuals were derived from average subpopulation response to either WR or WR. Parameters for captive individuals under PE were derived from average responses to this action in isolation. Parameters for MP were adjusted based on assumed additive effects.</p><p><strong>Note</strong> &mdash; To estimate cost of WR, please enter an appropriate number of wolves to be removed.</p><br/>"),
          ),
          column(width=6,
      ...
      tabItem("breeding1",
        fluidRow(
          column(width=12,
            ...
            HTML("<br/><p><strong>Limitations</strong> &mdash; Results using multiple levers are extrapolated based on knowledge from locations where single levers were studied. Combinations of these levers do not have documented examples and need to be treated with caution. Please see documentation on how combinations were parameterized.</p><p>CB = Conservation Breeding; MR = Moose Reduction; WR = Wolf Reduction</p><p>Parameters for non-captive individuals were derived from average subpopulation response to either WR or WR. Parameters for captive individuals under CB were derived from average responses to this action in isolation.</p><p><strong>Note</strong> &mdash; To estimate cost of WR, please enter an appropriate number of wolves to be removed.</p><br/>"),
            box(
              width = NULL, status = "success", solidHeader = TRUE,
              collapsible = FALSE, collapsed = FALSE,
              title = "Population forecast: conservation breeding",

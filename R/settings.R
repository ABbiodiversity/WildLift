caribou_settings <-
function(pen.type=c("mat.pen",  "pred.excl"), ...) {
    pen.type <- match.arg(pen.type)
    parms <- list()
    if (pen.type=="mat.pen") {
        ## costs
        parms$pen.cap <- 35       # how many individual caribou can live in mat pen?
#        parms$pen.cost1 <- 500    # cost in thousands to set up pen
#        parms$pen.cost2 <- 200    # cost in thousands for shepherd
#        parms$pen.cost3 <- 250    # cost in thousands to capture cows, monitor, survey, calf collar
#        parms$pen.cost4 <- 100    # unexpected contingency/repair costs
#        parms$pen.cost5 <- 80     # costs of  project manager
...
#        parms$pen.cost1 <- (77*24) + 20 # cost in thousands to set up pen, & xtra cost of initial pred removal in yr1
#        parms$pen.cost2 <- 80     # cost in thousands for removing predators annually
#        parms$pen.cost3 <- 500    # cost in thousands for patrolling and repairing fence
#        parms$pen.cost4 <- 200    # cost in thousands to capture cows, monitor, survey, calf collar
#        parms$pen.cost5 <- 100    # unexpected contingencies costs
